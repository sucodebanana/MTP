...
void modImagem(Pixel *imagem, HeaderBMP24bpp * pheader, int qtd_pixels,Pixel *mod)
{
    int j,i;
    byte lixo='a';
    FILE * arq;
for(j = 0; j < pheader->altura_px; j++) {
		...
		if(AUX > 0) // se não for múltiplo de 32 bits
			fwrite(&lixo,AUX,1,arq); // fwrite(&lixo, AUX, 1, arq);
	}
}
...
int main() {
	...
	imagem = carregar_imagem("super.bmp", &header);
	if(imagem) {
		modImagem(imagem, header.largura_px*header.altura_px,MOD);
		free(imagem);
	}
	//fwrite()
	return EXIT_SUCCESS;
